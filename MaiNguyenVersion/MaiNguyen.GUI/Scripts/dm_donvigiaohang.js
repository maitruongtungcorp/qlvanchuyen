var DM_DonViGiaoHang = function (options) {
    for (var property in options) {
        this[property] = options[property];
    }
    return this.Initialize(options);
}
DM_DonViGiaoHang.prototype = {
    ...
    RegisterEvent: function () {
        this.IsPaging = false;
        var that = this;
        if (!Common.Empty(Common.DM_DonViGiaoHang)) {
            that = Common.DM_DonViGiaoHang;
        }
        $("#ThanhPhoID").select2();
        //keyup hoten khach
        $("#form-update-khachhang").find("input[name='TenKH']").unbind("keyup").keyup(function () {
            var maKhachHang = Common.DM_DonViGiaoHang.CreateCodeKhachHang(Common.DM_DonViGiaoHang.TenDangNhap, $(this).val(), $("#form-update-khachhang").find("input[name='DienThoai']").val());
        ...
        $("#khachhang-search-form").find("select[name='SearchModel.PageSize']").unbind("change").change(function () {
            Common.DM_DonViGiaoHang.SubmitForm();
        })
        $(".delete-dm_donvigiaohang").unbind("click").click(function () {
                ...
                Items: {
                    Cancel: {
                        ...
                        OnClick: function (target) {
                            ...
                            Common.Ajax({
                                type: "POST",
                                url: Common.DM_DonViGiaoHang.Url.Delete,
                                async: false,
                                cache: false,
                                data: {
                                    id: id
    ...
    BeforeSend: function () {
        ...
        if (this.nodeName == "A") {
            page = Common.DM_DonViGiaoHang.GetCurrentPage($(this));
            $("#khachhang-search-form").find("input[name='SearchModel.PageNum']").val(page);
            Common.DM_DonViGiaoHang.SubmitForm();
            return false;
        }
    ...
    Success: function () {
        ...
        Common.DM_DonViGiaoHang.RegisterEvent();
    }
};
// create DM_LyDoThatBai Object
var DM_DonViGiaoHang = DM_DonViGiaoHang;
DM_DonViGiaoHang.constructor = DM_DonViGiaoHang;
