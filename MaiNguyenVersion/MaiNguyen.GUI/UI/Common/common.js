...
function InitSortingHeader(id) {
    ...
    $('#' + id + ' .table  thead  th').on("click", function (event) {
        ...
        //nguyen add
        //var dataAllList = $(this).attr("data-alllist");
        //var dataLastList = $(this).attr("data-lastlist");
        //
        ...
        if (action != "" && action != null) {
            ...
            //nguyen add
            //if (dataAllList != null || dataAllList != "")
            //{
            //    LoadTableDataAllList();
            //}
            //else if (dataLastList != null || dataLastList != "")
            //{
            //    LoadTableDataLastList();
            //}
            //else
            //{
            //    LoadTableData();
...
function DeleteConfirmationModal2InputCode(name, id, code) {
    var ID = id + "";
    var CODE = code + "";
    //fixed size modal
    $(".devoops-modal").css("width", 350);
    ...
    //$("#my-email").html(email);
    var html = "Bạn có thực sự muốn xóa - " + name + "?";
    var html2 = ' <button id="delete-ok-button" class="btn btn-success ladda-button"  data-style="slide-left" style="margin:0; width:80px;; float:right;" onClick="DeleteItem2InputCode(\'' + ID + '\',\'' + CODE + '\')";>Đồng ý</button><button id="delete-cancel-btn" class="btn btn-danger" style="margin:0;margin-right: 10px; width:80px; float:right;" onClick="CloseModalBox();">Hủy</button>';
...
function ShowFormModal(title, html, html2, w) {
   ...
   // var w2 = 600;//$(".devoops-modal").width();
    ...
    //var left = (w / 2) - (w2 / 2);
    //$(".devoops-modal").css('left', left + 'px');
    //height default của ashraf
    //$(".devoops-modal").css('top', top + 'px');
    $(".devoops-modal").css('top', 80 + 'px');
}
